...
def _get_minimal_slice_set(
    ...
    def reduce_edge_list(ls):
        tally = 1
        for i in range(len(ls)):
            reversed_idx = -1 * (i + 1)
            ls[reversed_idx] *= tally
            tally = ls[reversed_idx]
...
class ChunkSizeTuner:
    ...
    def _determine_favorable_chunk_size(self, fn, args, min_chunk_size):
        ...
        candidates = [
            2**log for log in range(int(math.log(self.max_chunk_size, 2)) + 1)
        ]
    ...
    def _compare_arg_caches(self, ac1, ac2):
        ...
        for a1, a2 in zip(ac1, ac2):
            ...
            if isinstance(ac1, list) or isinstance(ac1, tuple):
