...
def block_delete_msa(protein, config):
    ...
    del_block_starts = torch.randint(
        low=1, high=num_seq, size=(nb,), device=protein["msa"].device
    )
...
def make_masked_msa(protein, config, replace_fraction, seed):
    ...
    random_aa = torch.tensor(
        [0.05] * 20 + [0.0, 0.0], dtype=torch.float32, device=device
    )
...
def atom37_to_frames(protein, eps=1e-8):
    ...
    if is_multimer:
        gt_frames = gt_frames.compose_rotation(Rot3Array.from_array(rots))
    else:
