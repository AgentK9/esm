...
def _load_model_and_alphabet_core_v1(model_data):
    ...
    if model_data["args"].arch == "roberta_large":
        # upgrade state dict
        def pra(s):
            return "".join(s.split("encoder_")[1:] if "encoder" in s else s)

        def prs1(s):
            return "".join(s.split("encoder.")[1:] if "encoder" in s else s)

        def prs2(s):
            "".join(s.split("sentence_encoder.")[1:] if "sentence_encoder" in s else s)
        ...
        # upgrade state dict
        def pra(s):
            return "".join(s.split("decoder_")[1:] if "decoder" in s else s)

        def prs(s):
            return "".join(s.split("decoder.")[1:] if "decoder" in s else s)
        ...
        def prs3(s):
            return (
                s.replace("row", "column") if "row" in s else s.replace("column", "row")
            )
        ...
        if model_args.get("embed_positions_msa", False):
            ...
            model_args["embed_positions_msa_dim"] = (
