...
def atom14_to_atom37(
    ...
    atom37_data = tensor.batched_gather(
        atom14_data,
...
def atom37_to_atom14(aatype, all_atom_pos, all_atom_mask):
    ...
    atom14_mask = tensor.batched_gather(
        all_atom_mask,
...
def atom37_to_frames(
    ...
    base_atom_pos = tensor.batched_gather(
        all_atom_positions,
    ...
    gt_atoms_exist = tensor.batched_gather(  # shape (N, 8, 3)
    ...
    gt_frames = gt_frames.compose_rotation(
        Rot3Array.from_array(torch.tensor(rots, device=aatype.device))
    )
    ...
    ambiguity_rot = Rot3Array.from_array(
...
def torsion_angles_to_frames(
    ...
    # all_rots are Rot3Array with shape (..., N, 8)
    all_rots = Rot3Array(
        ones,
        zeros,
        zeros,
...
def frames_and_literature_positions_to_atom14_pos(
    aatype: torch.Tensor, all_frames_to_global: Rigid3Array  # (*, N)  # (N, 8)
) -> Vec3Array:  # (*, N, 14)
    ...
    # Vec3Array with shape (N, 14)
    lit_positions = Vec3Array.from_array(
...
def extreme_ca_ca_distance_violations(
    ...
    ca_ca_distance = euclidean_distance(this_ca_pos, next_ca_pos, eps)
    violations = (ca_ca_distance - rc.ca_ca) > max_angstrom_tolerance
    mask = this_ca_mask * next_ca_mask * has_no_gap_mask
    return tensor.masked_mean(mask=mask, value=violations, dim=-1)
...
def compute_chi_angles(positions: Vec3Array, mask: torch.Tensor, aatype: torch.Tensor):
    ...
    chi_angles = dihedral_angle(a, b, c, d)
    ...
    chi_angle_atoms_mask = tensor.batched_gather(
...
def make_transform_from_reference(
    a_xyz: Vec3Array, b_xyz: Vec3Array, c_xyz: Vec3Array
) -> Rigid3Array:
    ...
    """
    rotation = Rot3Array.from_two_vectors(c_xyz - b_xyz, a_xyz - b_xyz)
    return Rigid3Array(rotation, b_xyz)
